...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [productRes, collectionsRes, brandsRes] = await Promise.all([
          fetch(`/api/admin/products/${productId}`),
          fetch('/api/admin/collections'),
          fetch('/api/admin/brands'),
        ]);
        
        const [productData, collectionsData, brandsData] = await Promise.all([
          productRes.json(),
          collectionsRes.json(),
          brandsRes.json(),
        ]);
        ...
        if (productData.data) {
          ...
          setFormData({
            ...
            lead_time_weeks: product.lead_time_weeks.toString(),
            width_mm: product.width_mm?.toString() || '',
            depth_mm: product.depth_mm?.toString() || '',
            height_mm: product.height_mm?.toString() || '',
            weight_kg: product.weight_kg?.toString() || '',
            volume_m3: product.volume_m3?.toString() || '',
            pack_type: product.pack_type || '1PC/CTN',
            carton_length_mm: product.carton_length_mm?.toString() || '',
            ...
            stock_reserved: product.stock_reserved.toString(),
            stock_incoming: product.stock_incoming.toString(),
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="heading-2 text-gray-900">Edit Product</h1>
            <p className="body text-gray-600 mt-1">Loading product data...</p>
          </div>
        </div>
        <div className="card">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="heading-2 text-gray-900">Edit Product</h1>
          <p className="body text-gray-600 mt-1">Update product details</p>
        </div>
      </div>
