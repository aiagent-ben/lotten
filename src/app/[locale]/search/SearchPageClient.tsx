...
export default function SearchPageClient() {
  ...
  return (
    <main className="min-h-screen bg-background">
      {/* Search Header */}
      <section className="py-8 bg-stone-100/50 border-b border-stone-200">
              ...
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-4 text-lg bg-white border-2 border-stone-200 rounded-xl focus:border-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-600/20 shadow-xs"
                    autoFocus
                  />
                </div>
                <button
            ...
            <aside className="lg:w-64 flex-shrink-0">
              <div className="sticky top-24 space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className="heading-4 text-stone-900">Filters</h2>
                  <button
                ...
                <div className={cn('space-y-6', !showFilters && 'lg:hidden')}>
                      ...
                      {results?.facets.collection_id && Object.entries(results.facets.collection_id as Record<string, number>).map(([collectionId, count]) => (
                        <label key={collectionId} className="flex items-center gap-3 cursor-pointer">
                          <input
                            ...
                            className="w-4 h-4 text-amber-900 border-stone-300 rounded focus:ring-amber-600/20"
                          />
                          <span className="text-stone-700">{collectionId}</span>
                          <span className="ml-auto text-xs text-stone-400 tabular-nums">({count})</span>
                        </label>
                      ))}
                    </div>
                  </FilterSection>
                  ...
                  <FilterSection title="Price Range" icon={<Filter className="w-4 h-4" />}>
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        className="input px-3 py-2 text-sm border-stone-200"
                        min="0"
                        step="100"
                      />
                      <input
                  ...
                  <FilterSection title="Dimensions (mm)" icon={<Filter className="w-4 h-4" />}>
                    <div className="grid grid-cols-2 gap-2">
                      <input type="number" placeholder="Min W" value={filters.minWidth || ''} onChange={(e) => updateFilter('minWidth', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                      <input type="number" placeholder="Max W" value={filters.maxWidth || ''} onChange={(e) => updateFilter('maxWidth', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                      <input type="number" placeholder="Min D" value={filters.minDepth || ''} onChange={(e) => updateFilter('minDepth', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                      <input type="number" placeholder="Max D" value={filters.maxDepth || ''} onChange={(e) => updateFilter('maxDepth', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                      <input type="number" placeholder="Min H" value={filters.minHeight || ''} onChange={(e) => updateFilter('minHeight', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                      <input type="number" placeholder="Max H" value={filters.maxHeight || ''} onChange={(e) => updateFilter('maxHeight', e.target.value ? parseInt(e.target.value) : undefined)} className="input px-3 py-2 text-sm border-stone-200" />
                    </div>
                  </FilterSection>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <p className="text-stone-600">
                    {results ? (
                      <>Showing <strong className="tabular-nums">{(results.page - 1) * results.limit + 1}</strong>–<strong className="tabular-nums">{Math.min(results.page * results.limit, results.estimatedTotalHits)}</strong> of <strong className="tabular-nums">{results.estimatedTotalHits}</strong> products</>
                    ) : (
                      'Search for products...'
                    )}
                  </p>
              ...
              {loading && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[...Array(8)].map((_, i) => (
                    <div key={i} className="product-card bg-white rounded-xl border border-stone-200 overflow-hidden animate-pulse">
              ...
              {!loading && results && results.hits.length === 0 && (
                <div className="text-center py-16">
                  <Search className="mx-auto w-16 h-16 text-stone-300 mb-4" />
                  <h3 className="heading-3 text-stone-900 mb-2">No products found</h3>
  ...
  return (
    <details className="group">
      <summary className="flex items-center gap-2 cursor-pointer list-none">
        ...
        <ChevronDown className="ml-auto w-4 h-4 text-stone-400 transition-transform group-open:rotate-180" />
      </summary>
      <div className="mt-4 pb-4 border-b border-stone-200">
        {children}
      </div>
    </details>
  );
...
function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="product-card group bg-white rounded-xl border border-stone-200 overflow-hidden hover:shadow-lg transition-shadow duration-300" role="listitem">
      <Link href={`/products/${product.slug}`} className="block" aria-label={`View ${product.name}`}>
        ...
        <div className="relative aspect-square overflow-hidden bg-stone-100">
          ...
          <div className="absolute top-3 left-3 right-3 flex flex-col gap-1.5">
            {product.is_new && (
              <span className="badge-primary badge-success text-xs px-2.5 py-0.5 self-start">New</span>
            )}
            {product.is_bestseller && (
              <span className="badge-primary text-xs px-2.5 py-0.5 self-start">Bestseller</span>
            )}
            {product.stock_available > 0 && product.stock_available <= 5 && (
              <span className="badge-warning text-xs px-2.5 py-0.5 self-start">Low Stock</span>
            )}
          </div>
        </div>
        ...
        {/* Product Info */}
        <div className="p-4">
          <p className="caption text-amber-800 font-medium mb-1 uppercase tracking-wider">
